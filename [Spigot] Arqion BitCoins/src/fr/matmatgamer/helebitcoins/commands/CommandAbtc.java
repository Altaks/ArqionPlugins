	...
	public boolean onCommand(CommandSender sender, Command cmd, String msg, String[] args) {
		...
		if (args.length == 0 || args[0].equals("help")){
			...
			sender.sendMessage("§6=§c-§6=§c-§6=§c-§6=§c-§6=§c-§6=§c-§6=§c-§6=§c-§6=§c- §4Admin - HELP §c-§6=§c-§6=§c-§6=§c-§6=§c-§6=§c-§6=§c-§6=§c-§6=§c-§6=\n\n\n"
					...
					+ "§c/abtc reset <joueur> §9-> §7remet les bitcoins au joueur <joueur>  0\n\n\n"
			...
			if(args.length != 1) {
				...
				if(!file.exists()) {
					sender.sendMessage(Main.prefix + " §4Ce joueur ne s'est jamais connect� !");
					return false;
				}
				...
				if (args.length != 2) {
					...
					sender.sendMessage(Main.prefix + " §b" + given + " §6BitCoins ont �t� donn�s � §9" + player.getName() + "§6. Il poss�de maintenant §b" + now + " §6BitCoins");
					player.sendMessage(Main.prefix + " §b" + given + " §6BitCoins vous ont �t� donn�s par un §4Haut-Staff§6. Vous poss�dez maintenant §b" + now + " §6BitCoins");
				} else {
					sender.sendMessage(Main.prefix + " §4Veuillez sp�cifier un nombre de BitCoins !");
					return false;
				}
			...
			if(args.length != 1) {
				...
				if (!player.isOnline()) {
					sender.sendMessage(Main.prefix + " §4Le joueur doit �tre en ligne !");
					return false;
				}
				...
				if (args.length != 2) {
					...
					sender.sendMessage(Main.prefix + " §b" + taken + " §6BitCoins ont été retiré à §9" + player.getName() + "§6. Il possède maintenant §b" + now + " §6BitCoins");
					player.sendMessage(Main.prefix + " §b" + taken + " §6BitCoins vous ont été retirés par un §4Haut-Staff§6. Vous possedez maintenant §b" + now + " §6BitCoins");
				} else {
