		...
		if (cmd.getName().equalsIgnoreCase("spawn")) {
			if (sender instanceof Player) {
				if (args.length == 0) {
					...
					p.teleport(spawn);
					p.sendMessage(Main.prefix + "§aVous avez été téléporté au spawn");
				} else sender.sendMessage(Main.prefix + "cPour vous téléporter au spawn, exétutez la commande '/spawn'");
